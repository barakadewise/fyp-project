...
export class YouthService {
  constructor(
    ...
    private readonly youthRepository: Repository<Youth>,
  ...
  async findOneByEmailOrPhone(identifier: string): Promise<any> {
    return await this.youthRepository
  ...
  async updateYouth(
    updateYouthDto: UpdateYouthDto,
    youthId: number,
  ): Promise<ResponseDto> {
    const youth = await this.youthRepository.findOne({
      where: { id: youthId },
    });
    if (youth) {
